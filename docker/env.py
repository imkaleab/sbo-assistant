...
"""
SBO Gas Station RL Env – simple version, no numpy required for smoke test
"""
import random, math
...
class GasStationProcurementEnv:
    ...
    def step(self, action):
        ...
        for tid, tank in self.state.tanks.items():
            ...
            # Only allow new PO if no pending (realistic) unless critical - prevents unbounded accumulation
            if order>0 and (tank.pending_po_gal==0 or tank.level_pct<10):
            ...
            traces.append({'tank_id':tid,'hour':self.hour,'before_gal':before,'action_gal':order,'after_gal':tank.current_gal,'pct':tank.level_pct,'stockout':tank.current_gal==0,'ctx':f"Tank {tid} at {tank.level_pct:.1f}% - {'CRITICAL' if tank.level_pct<25 else 'reorder' if tank.level_pct<35 else 'healthy'}"})
        self.hour+=1
    ...
    def render(self):
        ...
        for tid,t in self.state.tanks.items():
            flag="🚨" if t.level_pct<25 else "⚠️" if t.level_pct<35 else "✅"
            print(f"  {flag} {tid}: {t.current_gal:.0f}/{t.capacity} ({t.level_pct:.1f}%) pend {t.pending_po_gal} ETA {t.pending_po_eta_hr}h")

if __name__=='__main__':
    env=GasStationProcurementEnv(max_hours=24*7)
    env.reset()
    total=0
    for h in range(24*7):
        act={tid: random.choice(env.gal_options) for tid in TANK_CONFIGS}
        ...
        if done:
            break
    print(f"Total reward random: {total:.0f}")
